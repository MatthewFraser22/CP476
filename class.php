    ...
    <div class="container">
        ...
        <div style="overflow-x:auto;">
            <table>
                ...
                $rows = $db
                    ->query(
                        'SELECT *  FROM courses INNER JOIN students ON students.student_id = courses.student_id
                ...
                function postHelper($i)
                {
                    ...
                    $rows = $db
                        ->query(
                            "UPDATE students SET name=:name WHERE student_id = :id",
                            $data
                        )
                        ->fetchAll();
                    header("Refresh:0");
                    ...
                    <td  onclick='redirect(`{$row["student_id"]}`, `{$_GET["course"]}`)'>{$row["name"]}</td>
                     <td><button class='edit blueButton' type='button' onclick='showDiv($x - 1)'>Edit</button>
                        <form method='post' class='editForm' style='visibility: hidden; display:inline;'>
                            ...
                            <input  onclick='event.stopPropagation()' onkeydown='return /[a-z, ]/i.test(event.key)'  name='name{$x}'' placeholder='Enter characters' />
